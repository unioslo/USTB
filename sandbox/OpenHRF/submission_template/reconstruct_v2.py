"""Reconstruct B-mode images using zea.Pipeline with pressure-field weighting.

This is v2 of the reconstruction script. Instead of custom scanline/STA code
paths, it uses a single ``zea.Pipeline`` with ``enable_pfield=True`` for all
acquisition types. The pressure-field weighting simulates each transmit's
acoustic field and down-weights pixels outside the insonified region — handling
focused imaging (FI) and synthetic transmit aperture (STA) automatically.

Usage::

    python reconstruct_v2.py                   # all .hdf5 files recursively
    python reconstruct_v2.py --input file.hdf5
"""
...
def reconstruct_file(path: Path, f_number: float = 1.75, dynamic_range: float = 60.0,
                     grid_size_x: int = 400, grid_size_z: int = 600,
                     num_patches: int = 200, out_dir: Path | None = None) -> Path:
    """Reconstruct a single file using the pfield-enabled pipeline."""
    if out_dir is not None:
        out_dir.mkdir(parents=True, exist_ok=True)
        out_path = out_dir / (path.stem + "_reconstruct_v2_bmode.png")
    else:
        out_path = path.with_name(path.stem + "_reconstruct_v2_bmode.png")
    ...
    with File(str(path)) as f:
        ...
        # Phased-array sector scans are beamformed on a polar grid (following the
        # scan's steered angles) so the full sector field of view is preserved, then
        # scan-converted to cartesian for display. Linear/curved arrays use a
        # cartesian grid directly.
        is_phased = (probe_type == "phased")
        # Phased-array sectors use the full receive aperture (f-number 0): the small
        # aperture cannot satisfy a finite f-number at wide steering angles, which would
        # otherwise mask the edges of the sector and narrow the field of view.
    ...
    # Build pipeline with pfield enabled — this handles FI and STA automatically.
    # Preprocessing (band-pass, window, downsample) follows the zea cardiac example
    # gist (tristan-deep/067e80f28086ec894513e46422a7bbd1). For baseband (IQ) data the
    # band-pass and demodulation steps are skipped (already at baseband).
    fc = float(np.array(parameters.center_frequency))
    is_baseband = (n_ch == 2)
    # pfield weighting compounds multiple transmits by their simulated pressure fields.
    # With a single transmit (n_tx == 1, e.g. a single plane wave as in the SWE data)
    # there is nothing to compound, and pfield's percentile threshold instead zeros
    # low-pressure edge regions, producing hard black notches at the image edges. Only
    # enable pfield when there is more than one transmit.
    use_pfield = n_tx > 1
    ops_list = [Cast(dtype="float32")]
    if not is_baseband:
        ops_list.append(BandPassFilter(passband=(0.5 * fc, 1.5 * fc)))
    ops_list.append(ApplyWindow())
    if not is_baseband:
        ops_list.append(Demodulate())
    ops_list += [
        Downsample(factor=2),
        Beamform(beamformer="delay_and_sum", enable_pfield=use_pfield, num_patches=num_patches),
        EnvelopeDetect(),
        Normalize(),
        LogCompress(),
    ]
    pipeline = Pipeline(operations=ops_list, with_batch_dim=False)
    ...
    if is_phased:
        # Polar grid: map each beamformed pixel to its true cartesian position so the
        # full steered sector (all transmit angles) is rendered without clipping.
    ...
    ax.set_xlabel("x (m)")
    ax.set_ylabel("z (m)")
    ax.set_title("zea (pfield): " + path.stem, fontsize=8)
...
def main():
    ...
    if not files:
        ...
        return
    print(f"Reconstructing {len(files)} file(s) with pfield-enabled pipeline...")
    for path in files:
        try:
            reconstruct_file(
                path,
