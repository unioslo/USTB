...
%% Do beamforming
depth_axis=linspace(0e-3,75e-3,512).';
...
mid_RTB.transmit_apodization.window = uff.window.tukey25; % dette var tukey50 
mid_RTB.transmit_apodization.f_number = 4;
mid_RTB.transmit_apodization.minimum_aperture = 3*[10e-3 10e-3];
...
%%
mid_RTB.transmit_apodization.window = uff.window.scanline;
mid_RTB.transmit_apodization.MLA_overlap = 0;
b_data_delayed_MLA = mid_RTB.go();
CF.input = b_data_delayed_MLA;
b_data_CF_MLA = CF.go()
CF_MLA = uff.beamformed_data(CF.CF);

f = figure;
CF_MLA.plot(subplot(121),['Coherence Factor from MLA'],[],['none'])
colormap default
xlim([-30 30])
caxis([0 1])

CF_RTB.plot(subplot(122),['Coherence Factor from RTB'],[],['none'])
colormap default
xlim([-30 30])
caxis([0 1])
set(gcf,'Position',[325 374 1044 460])
saveas(f,'Figures/CF_example.png');
%%
...
% %     seq
% %     channel_data_RF.sequence(seq).probe = channel_data.probe 
% %     %= ones(1,channel_data.N_channels);
% % end
% % %%
...
% mid_STAI.receive_apodization.window=uff.window.tukey25;
