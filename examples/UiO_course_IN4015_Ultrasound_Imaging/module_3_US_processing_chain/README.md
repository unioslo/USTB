...
scratch. In Module 2, we used pixel based beamforming, but only did receive 
...
## Literature
The relevant literature for this assignment is the lecture slides. Especially the
...
well as 1.9. However, the other sections prior to 1.5 are also very relevant.
...
**Deadline for uploading: Tuesday 29. September 2026 at 10:00.**

## Datasets
You have two available datasets you can use for this exercise

+ Verasonics_P2-4_parasternal_long_small.uff which is an in-vivo cardiac dataset
+ FieldII_P4_point_scatterers.uff which is a simulated dataset of point scatterers
...
in the lab. More information regarding this will be given in the lecture and the group sessions.

The two datasets can be downloaded directly from (if you have issues with the automatic download in the USTB):

+ https://zenodo.org/records/20261898/files/FieldII_P4_point_scatterers.uff?download=1
+ https://zenodo.org/records/20261898/files/Verasonics_P2-4_parasternal_long_small.uff?download=1
...
on the geometry of beambased beamforming for tips on how to implement this.
...
Pseudocode for the beambased phased array beamformer you will implement
    ...
    for each transmit
        ...
        for each receive channel
            ...
            interpolating (using interp1) for each depth sample for each transmit
            ...
            interp1(sample_time, rfData(:,r,t)', delays(t,:,r))'
                ...
                you already have sample_time and rfData, and need to calculate the delays
