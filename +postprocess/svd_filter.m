classdef svd_filter < postprocess
        % SVD_FILTER Apply SVD spatiotemporal clutter filter to Beamformed data
    ...
    %
    % See also POSTPROCESS, UFF.BEAMFORMED_DATA, SVD
    ...
    methods(Access = public)
        function [output] = go(h)
            ...
            %   and returns the filtered data as a beamformed_data object.
            %
            ...
            %
            %   Input data is accessed through h.input (uff.beamformed object)
            ...
            if or(isequal(h.cutoff,1:initsize(end)),h.cutoff(1)<2)
                h.output = uff.beamformed_data(h.input); % Just copy over data
                output = h.output;
                return
            end
