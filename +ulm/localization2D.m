function MatTracking = localization2D(MatIn,ULM)
%% function MatTracking = localization2D(MatIn,ULM)
...
N_z = size(index_mask_z,1);

for iscat=1:N_z
    if mod(iscat, 100) == 0 && ULM.workbars
        tools.workbar(iscat/N_z, strjoin(["Computing scatters... [" iscat "/" N_z "]"], 'ULM Localization'));
    end
...
%% function [Zc,Xc,sigma] = LocRadialSym(Iin,fwhm_z,fwhm_x)
    [Zc,Xc] = ulm.localizeRadialSymmetry(Iin,fwhm_z,fwhm_x);
    sigma = ComputeSigmaScat(Iin,Zc,Xc);
end

function [Zc,Xc,sigma] = LocRadialSym_sg(Iin,fwhm_z,fwhm_x)
%% function [Zc,Xc,sigma] = LocRadialSym(Iin,fwhm_z,fwhm_x)
    [Zc,Xc] = ulm.localizeRadialSymmetry_sg(Iin,fwhm_z,fwhm_x);
    sigma = ComputeSigmaScat(Iin,Zc,Xc);
end
